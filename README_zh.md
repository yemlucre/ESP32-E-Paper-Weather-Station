...
1. 屏幕只显示红色或丢失黑色
   - 检查是否使用了 `GxEPD2_3C` 而非 `GxEPD2_BW`。
   - 确保使用 `GxEPD_BLACK`、`GxEPD_RED` 等颜色常量。

2. 屏幕刷新后模糊
   - 确认 BUSY 引脚是否正确接线与可用。
   - 强制全屏刷新（本项目采用 `setFullWindow()`）。

3. WiFi 连接不上
   - 检查 `include/config.h` 中的 SSID/密码是否正确。
   - 路由器隔离或 MAC 过滤等网络策略可能阻止连接。

4. WeatherAPI 返回错误或配额不足
   - 检查 `WEATHER_API_KEY` 是否有效
   - 增加请求超时时间或重试次数以应对网络波动

测试建议
--------

- 在开发时使用较短的更新时间间隔以便快速调试（但不要长期保持短间隔以免触发 API 限制）。
- 可在 `setup()` 中加入示例 `WeatherInfo` 测试渲染以离线验证排版。
- 用示波器或逻辑分析仪检查 SPI 信号（仅在硬件故障怀疑时需要）。

局限与改进建议
---------------

- 频繁的全屏刷新对功耗不友好，不适合电池供电场景。改进：
  - 增大更新间隔（例如每 10 分钟），或实现仅部分区域刷新（若硬件支持）。
- 显示代码耦合较高，建议实现一个简单的布局引擎或模板系统以便维护。
- 添加 OTA（远程固件升级）功能便于实验室或课程部署后的维护。

附录：文件清单与关键代码片段
---------------------------

- `platformio.ini` — 构建配置（platform：espressif32，board：esp32dev）
- `include/config.h` — 引脚与常量配置（请在论文中保留并注明不可公开的 API Key）
- `include/weather_api.h` / `src/weather_api.cpp` — WeatherAPI 请求与 JSON 解析
- `include/dht_sensor.h` / `src/dht_sensor.cpp` — DHT 读取与 upload 钩子
- `include/epaper_display.h` / `src/epaper_display.cpp` — 三色屏驱动与渲染
